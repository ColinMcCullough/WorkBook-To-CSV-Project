...
function clearDashboardContent() {
  dashboardSheet.getRange('A3:H500').clearContent();
}
/*
...
function getKeywords(tableLocationInfoArry,locVert) {
  ...
  if(hasAddressVal(tableLocationInfoArry)) {
    ...
    for(var k = 0; k < type2.keys.length; k++) {
      ...
      wholeArry = pushLocationNames(latAndLong,businessType,numberToGet,wholeArry,type2.typeSet);
    }
    for(var k = 0; k < type1.keys.length; k++) {
      var businessType = type1.keys[k];
      var numberToGet = type1.typeMap[type1.keys[k]];
      wholeArry = pushLocationNames(latAndLong,businessType,numberToGet,wholeArry,type1.typeSet);
    }
...
//checks the table in ui to make sure a address exists
...
function getClientKeywords(locationTable) {
  ...
  for(var i = 0; i < loopLen; i++) {
    row.push([returnIndexValOrBlank(locNeighborhoodTermsArry,i),returnIndexValOrBlank(locLandmarkTermsArr,i)]);                                                        
  }
  return row;
}

function getLoopLen(locNeighborhoodTermsArry,locLandmarkTermsArr) {
   if(locNeighborhoodTermsArry.length > locLandmarkTermsArr.length) {
    return locNeighborhoodTermsArry.length;
  } else {
    return locLandmarkTermsArr.length;
  }
}
  
function returnIndexValOrBlank(array,i) {
   if(array[i] != null) {
      return array[i];
   } else {
      return "";
...
function structureResults(data,city) {
  var newData = [];
  for (var i in data) {
    var row = data[i];
    var duplicate = false;
...
@return {[lat,long]} returns an array with lat,long
*/
function getLatLong(address) {
  //Rest API call that returns lat and long for a given address in JSON
  var addrurl = 'https://maps.googleapis.com/maps/api/geocode/json?'
      + 'address='+address
      + '&key=AIzaSyD0qNg1CyA0l4_m_reeJP7JbKPUL5wway4';
    
  //Post the call
  var addrresponse = UrlFetchApp.fetch(addrurl);
  //Parse the JSON that was returned
  var addrdata = JSON.parse(addrresponse);
    
  //get the lat long
  var lat = Object(addrdata.results[0].geometry.location.lat);
  var lng = Object(addrdata.results[0].geometry.location.lng);
  return [lat,lng];
}


function pushLocationNames(latAndLong,typesBus, printHowMany, wholeArry,typeSet) {
  var lat = latAndLong[0];
  var lng = latAndLong[1];

  var url;
  //Rest API call that return JSON
  if(typeSet === 'neighborhood') {
  ...
  //Count number of objects in results since no Length method is programmed into Object
  var count = Object.keys(data.results).length;
    
  if(count >= printHowMany){
    count=printHowMany;
  }
  for(i=0;i<count;i++){
    var locationName = "";
    locationName = data.results[i].name;
    wholeArry.push([typesBus,locationName]);
  }
  return wholeArry;
}
...
var neighborhoodMap = {
  ...
  "colloquial_area" : "4", "intersection" : "4", 
  "locality" : "4", 
  "neighborhood" : "4", 
  "political" : "4", 
  "route" : "4", 
...
function Type(typeSet ,vertical, class){
  // Add object properties
   this.class = class;
   this.vertical = vertical;
   this.typeSet = typeSet;
   // sets up Typemap
   this.setTypeMap = function(typeSet,vertical,class) {
     if(typeSet === "neighborhood") {
       typeMap = neighborhoodMap;
       return typeMap;
     } 
     else if(typeSet === "landmark") {
       if(vertical === "mf" && class === "Class A") {typeMap = mfClassAMap;}
       else if(vertical === "mf" && class === "Class B") {typeMap = mfClassBMap;}
       else if(vertical === "mf" && class === "Class C") {typeMap = mfClassCMap;}
       else if(vertical === "ss") {typeMap = ssMap;}
