...
function setLVHeaderFormatting(vertical,domainType,tabToFormat) {
  ...
  var strategiesRange = tabToFormat.getRange(2,headerRange[0].indexOf("pr_notes") + 2,3,7);
...
function defineDataValidation(uspsCol,accOrRejecCol,gmbColumn,gaColumn,redirectsColumn,prColumn,propFeatCol,tabToFormat,headerRange) {
  ...
  var gmbVal = SpreadsheetApp.newDataValidation().requireValueInList(["Requested","Accessed","Create New","Unverified","N/A Basic Package","N/A - Client Managed"], true).build(); //gmb validation builder
